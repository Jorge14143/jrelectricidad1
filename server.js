...
app.post("/api/public/quotes/:token/accept", authLimiter, async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) {
      ...
      // Crear el trabajo asociado al presupuesto aceptado
      const [acceptedQuoteRows] = await pool.query(
        `
        SELECT id
        FROM quotes
        WHERE access_token = ?
      ...
      if (acceptedQuoteRows.length) {
        await pool.query(
          ...
          ON DUPLICATE KEY UPDATE
            status = 'aceptado'
          `,
          [acceptedQuoteRows[0].id]
        );
      }
    // Obtener información del presupuesto para la notificación
...
app.post("/api/public/quotes/:token/reject", authLimiter, async (req, res) => {
  try {
    ...
    // Crear el trabajo asociado al presupuesto rechazado
      const [rejectedQuoteRows] = await pool.query(
        `
        SELECT id
        FROM quotes
        WHERE access_token = ?
