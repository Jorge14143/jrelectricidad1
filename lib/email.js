...
${data.link ? `<p>Este enlace vence en ${Number(data.minutes || 30)} minutos y solo puede utilizarse una vez.</p>` : ""}`)
    ...
    case "request_received":
      return {
        subjectFallback: "Solicitud recibida - JR Electricidad",
        html: layout("Solicitud recibida", `
<p>Hola ${escapeHtml(data.name || "")},</p>
<p>Recibimos correctamente tu solicitud de presupuesto.</p>
<p><strong>Solicitud #${escapeHtml(data.requestId || "")}</strong></p>
<p>Servicio: ${escapeHtml(data.service || "Consulta")}</p>
<p>Te contactaremos cuando revisemos la solicitud.</p>
<p>Saludos,<br>JR Electricidad · Electricista Matriculado Cat. 3</p>`)
      };
    case "request_status":
      return {
        subjectFallback: "Actualización de solicitud - JR Electricidad",
        html: layout("Actualización de solicitud", `
<p>Hola ${escapeHtml(data.name || "")},</p>
<p>Tu solicitud <strong>#${escapeHtml(data.requestId || "")}</strong> cambió de estado.</p>
<p>Nuevo estado: <strong>${escapeHtml(String(data.status || "").replace(/_/g, " "))}</strong></p>
<p>Saludos,<br>JR Electricidad · Electricista Matriculado Cat. 3</p>`)
      };
    case "job_update":
      return {
        subjectFallback: "Actualización de trabajo - JR Electricidad",
        html: layout("Actualización de trabajo", `
<p>Hola ${escapeHtml(data.name || "")},</p>
<p>El trabajo <strong>#${escapeHtml(data.jobId || "")}</strong> fue actualizado.</p>
<p>Estado: <strong>${escapeHtml(String(data.status || "").replace(/_/g, " "))}</strong></p>
${data.scheduledAt ? `<p>Programado para: <strong>${escapeHtml(data.scheduledAt)}</strong></p>` : ""}
<p>Saludos,<br>JR Electricidad · Electricista Matriculado Cat. 3</p>`)
      };
    case "quote_sent":
      return {
