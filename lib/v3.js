...
function createV3Router(express, pool) {
  ...
  // =========================================================
  async function v3InventoryEvent(itemId, actor, type, quantity = 0, metadata = {}) {
    await pool.query(
      "INSERT INTO v3_inventory_events(item_id,actor_user_id,event_type,quantity,metadata) VALUES(?,?,?,?,?)",
      [itemId || null, actor || null, type, quantity, JSON.stringify(metadata)]
    );
  }
  ...
  router.get("/admin/inventory/summary", async (req, res) => {
    if (!v3Admin(req, res)) return;
    try {
      const [[items]] = await pool.query("SELECT COUNT(*) total FROM v3_inventory_items WHERE active=1");
      const [[low]] = await pool.query("SELECT COUNT(*) total FROM v3_inventory_items WHERE active=1 AND stock_quantity<=min_stock");
      const [[value]] = await pool.query("SELECT COALESCE(SUM(stock_quantity*unit_cost),0) total FROM v3_inventory_items WHERE active=1");
      const [[suppliers]] = await pool.query("SELECT COUNT(*) total FROM v3_inventory_suppliers WHERE active=1");
      res.json({ ok: true, summary: { items: Number(items.total), low_stock: Number(low.total), stock_value: Number(value.total), suppliers: Number(suppliers.total) } });
    } catch (error) {
      console.error("V3 inventory summary:", error);
      res.status(500).json({ error: "No se pudo cargar el resumen de inventario." });
    }
  });

  router.get("/admin/inventory/items", async (req, res) => {
    if (!v3Admin(req, res)) return;
    try {
      const search = clean(req.query.search, 150);
      const category = clean(req.query.category, 100);
      const low = req.query.low === "1";
      const where = ["i.active=1"], params = [];
      if (search) {
        where.push("(i.sku LIKE ? OR i.name LIKE ? OR i.description LIKE ?)");
        const q = "%" + search + "%"; params.push(q, q, q);
      }
      if (category) { where.push("i.category=?"); params.push(category); }
      if (low) where.push("i.stock_quantity<=i.min_stock");
      const [items] = await pool.query(
        "SELECT i.*,s.name supplier_name FROM v3_inventory_items i LEFT JOIN v3_inventory_suppliers s ON s.id=i.supplier_id WHERE " +
        where.join(" AND ") + " ORDER BY i.name ASC LIMIT 1000", params
      );
      res.json({ ok: true, items });
    } catch (error) {
      console.error("V3 inventory items:", error);
      res.status(500).json({ error: "No se pudieron cargar los artículos." });
    }
  });

  router.post("/admin/inventory/items", async (req, res) => {
    if (!v3Admin(req, res)) return;
    const name = clean(req.body.name, 190);
    const sku = clean(req.body.sku, 80) || null;
    const category = clean(req.body.category, 100) || null;
    const unit = clean(req.body.unit, 30) || "unidad";
    ...
    if (!name || !Number.isFinite(unitCost) || unitCost < 0 || !Number.isFinite(minStock) || minStock < 0 || !Number.isFinite(initialStock) || initialStock < 0) {
      return res.status(400).json({ error: "Datos de artículo inválidos." });
    }
    try {
      const [r] = await pool.query(
        "INSERT INTO v3_inventory_items(sku,name,description,category,unit,unit_cost,min_stock,stock_quantity,supplier_id,active,created_by) VALUES(?,?,?,?,?,?,?,?,?,1,?)",
        [sku, name, clean(req.body.description, 1000) || null, category, unit, unitCost, minStock, initialStock, supplierId, req.session.user.id]
      );
      if (initialStock) {
        await pool.query(
          "INSERT INTO v3_inventory_movements(item_id,type,quantity,unit_cost,reference,notes,created_by) VALUES(?,?,?,?,?,?,?)",
          [r.insertId, "entrada", initialStock, unitCost, "ALTA-INICIAL", "Stock inicial", req.session.user.id]
        );
        await v3InventoryEvent(r.insertId, req.session.user.id, "initial_stock", initialStock, {});
      }
      res.status(201).json({ ok: true, id: r.insertId });
    } catch (error) {
      if (error?.code === "ER_DUP_ENTRY") return res.status(409).json({ error: "El SKU ya existe." });
      console.error("V3 inventory create:", error);
      res.status(500).json({ error: "No se pudo crear el artículo." });
    }
  });

  router.patch("/admin/inventory/items/:id", async (req, res) => {
    if (!v3Admin(req, res)) return;
    const id = Number(req.params.id), fields = [], params = [];
    const allowed = ["sku","name","description","category","unit","unit_cost","min_stock","supplier_id","active"];
    for (const key of allowed) if (Object.prototype.hasOwnProperty.call(req.body, key)) {
      fields.push(key + "=?"); params.push(req.body[key] === "" ? null : req.body[key]);
    }
    if (!fields.length) return res.status(400).json({ error: "No hay cambios para guardar." });
    try {
      await pool.query("UPDATE v3_inventory_items SET " + fields.join(",") + ",updated_at=NOW() WHERE id=?", params.concat(id));
      await v3InventoryEvent(id, req.session.user.id, "updated", 0, { fields: fields.map(x => x.split("=")[0]) });
      res.json({ ok: true });
    } catch { res.status(500).json({ error: "No se pudo actualizar el artículo." }); }
  });

  router.get("/admin/inventory/suppliers", async (req, res) => {
    if (!v3Admin(req, res)) return;
    try {
      const [suppliers] = await pool.query("SELECT * FROM v3_inventory_suppliers WHERE active=1 ORDER BY name ASC");
      res.json({ ok: true, suppliers });
    } catch { res.status(500).json({ error: "No se pudieron cargar los proveedores." }); }
  });

  router.post("/admin/inventory/suppliers", async (req, res) => {
    if (!v3Admin(req, res)) return;
    const name = clean(req.body.name, 190);
    if (!name) return res.status(400).json({ error: "El proveedor es obligatorio." });
    try {
      const [r] = await pool.query(
        "INSERT INTO v3_inventory_suppliers(name,tax_id,phone,email,address,notes,active,created_by) VALUES(?,?,?,?,?,?,1,?)",
        [name, clean(req.body.tax_id, 40) || null, clean(req.body.phone, 50) || null, clean(req.body.email, 190) || null, clean(req.body.address, 255) || null, clean(req.body.notes, 1000) || null, req.session.user.id]
      );
      res.status(201).json({ ok: true, id: r.insertId });
    } catch { res.status(500).json({ error: "No se pudo crear el proveedor." }); }
  });

  router.post("/admin/inventory/movements", async (req, res) => {
    if (!v3Admin(req, res)) return;
    const itemId = Number(req.body.item_id), type = clean(req.body.type, 20);
    const quantity = Number(req.body.quantity), unitCost = Number(req.body.unit_cost || 0);
    const valid = ["entrada","salida","ajuste"];
    if (!itemId || !valid.includes(type) || !Number.isFinite(quantity) || quantity <= 0) return res.status(400).json({ error: "Movimiento inválido." });
    try {
      const [[item]] = await pool.query("SELECT id,name,stock_quantity,unit_cost FROM v3_inventory_items WHERE id=? AND active=1 FOR UPDATE", [itemId]);
      if (!item) return res.status(404).json({ error: "Artículo no encontrado." });
      let delta = type === "salida" ? -quantity : quantity;
      if (type === "ajuste") delta = Number(req.body.target_stock) - Number(item.stock_quantity);
      const next = Number(item.stock_quantity) + delta;
      if (next < 0) return res.status(409).json({ error: "Stock insuficiente." });
      const cost = Number.isFinite(unitCost) && unitCost >= 0 ? unitCost : Number(item.unit_cost || 0);
      await pool.query(
        "INSERT INTO v3_inventory_movements(item_id,type,quantity,unit_cost,reference,notes,job_id,created_by) VALUES(?,?,?,?,?,?,?,?)",
        [itemId, type, Math.abs(delta), cost, clean(req.body.reference, 120) || null, clean(req.body.notes, 1000) || null, Number(req.body.job_id || 0) || null, req.session.user.id]
      );
      await pool.query("UPDATE v3_inventory_items SET stock_quantity=?,unit_cost=CASE WHEN ? > 0 THEN ? ELSE unit_cost END,updated_at=NOW() WHERE id=?", [next, type === "entrada" ? cost : 0, cost, itemId]);
      await v3InventoryEvent(itemId, req.session.user.id, type, delta, { job_id: Number(req.body.job_id || 0) || null });
      res.status(201).json({ ok: true, stock_quantity: next });
    } catch (error) {
      console.error("V3 inventory movement:", error);
  ...
  router.get("/admin/inventory/movements", async (req, res) => {
    ...
    try {
      ...
      const params = [];
      let where = "";
      if (itemId) { where = " WHERE m.item_id=?"; params.push(itemId); }
      const [movements] = await pool.query(
        "SELECT m.*,i.name item_name,i.sku,j.job_number,u.name created_by_name FROM v3_inventory_movements m JOIN v3_inventory_items i ON i.id=m.item_id LEFT JOIN jobs j ON j.id=m.job_id LEFT JOIN users u ON u.id=m.created_by" +
        where + " ORDER BY m.created_at DESC,m.id DESC LIMIT 1000", params
      );
      res.json({ ok: true, movements });
    } catch { res.status(500).json({ error: "No se pudo cargar el historial." }); }
  });

  router.get("/admin/inventory/report", async (req, res) => {
    if (!v3Admin(req, res)) return;
    try {
      const [categories] = await pool.query("SELECT COALESCE(category,'Sin categoría') category,COUNT(*) items,SUM(stock_quantity) stock,SUM(stock_quantity*unit_cost) value FROM v3_inventory_items WHERE active=1 GROUP BY category ORDER BY value DESC");
      const [lowStock] = await pool.query("SELECT id,sku,name,stock_quantity,min_stock,unit,unit_cost FROM v3_inventory_items WHERE active=1 AND stock_quantity<=min_stock ORDER BY stock_quantity ASC,name ASC");
      res.json({ ok: true, report: { categories, low_stock: lowStock } });
    } catch { res.status(500).json({ error: "No se pudo generar el reporte de inventario." }); }
  });

  router.post("/admin/inventory/items/:id/consume-for-job", async (req, res) => {
    if (!v3Admin(req, res)) return;
    const itemId = Number(req.params.id), jobId = Number(req.body.job_id), quantity = Number(req.body.quantity);
    if (!itemId || !jobId || !Number.isFinite(quantity) || quantity <= 0) return res.status(400).json({ error: "Consumo inválido." });
    try {
      const [[item]] = await pool.query("SELECT id,stock_quantity,unit_cost FROM v3_inventory_items WHERE id=? AND active=1 FOR UPDATE", [itemId]);
      if (!item) return res.status(404).json({ error: "Artículo no encontrado." });
      if (Number(item.stock_quantity) < quantity) return res.status(409).json({ error: "Stock insuficiente." });
      await pool.query("INSERT INTO v3_inventory_movements(item_id,type,quantity,unit_cost,reference,notes,job_id,created_by) VALUES(?,?,?,?,?,?,?,?)",
        [itemId, "salida", quantity, Number(item.unit_cost || 0), "TRABAJO-" + jobId, clean(req.body.notes, 1000) || "Consumo para trabajo", jobId, req.session.user.id]);
      await pool.query("UPDATE v3_inventory_items SET stock_quantity=stock_quantity-?,updated_at=NOW() WHERE id=?", [quantity, itemId]);
      await pool.query("INSERT INTO v3_job_costs(job_id,kind,description,quantity,unit_price,total,created_by) VALUES(?,?,?,?,?,?,?)",
        [jobId, "material", item.name || "Material", quantity, Number(item.unit_cost || 0), +(quantity * Number(item.unit_cost || 0)).toFixed(2), req.session.user.id]).catch(async () => {
          await pool.query("INSERT INTO v3_job_costs(job_id,kind,description,quantity,unit_price,total,created_by) VALUES(?,?,?,?,?,?,?)",
            [jobId, "material", "Material de inventario #" + itemId, quantity, Number(item.unit_cost || 0), +(quantity * Number(item.unit_cost || 0)).toFixed(2), req.session.user.id]);
        });
      await v3InventoryEvent(itemId, req.session.user.id, "job_consumption", -quantity, { job_id: jobId });
      res.json({ ok: true });
    } catch (error) {
      console.error("V3 inventory job consumption:", error);
      res.status(500).json({ error: "No se pudo descontar el material del trabajo." });
    }
  });
