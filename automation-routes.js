function registerAutomationRoutes({app,pool,requireAdmin}){
 ...
 async function log(rule,event,target,status,message){
  await pool.query("INSERT INTO automation_logs(rule_id,event_key,target_type,target_id,status,message) VALUES(?,?,?,?,?,?)",[rule?.id||null,event,target?.type||null,target?.id||null,status,String(message||"").slice(0,1000)]).catch(()=>{});
 }

 async function executeRule(rule,eventKey,target){
  try{
   const c=typeof rule.config==="string"?JSON.parse(rule.config||"{}"):(rule.config||{});
   if(rule.action_type==="notification"){
    const msg=String(c.message||target.message||("Automatización: "+eventKey));
    const [dup]=await pool.query("SELECT id FROM admin_notifications WHERE type='system' AND quote_id <=> ? AND message=? AND created_at>=DATE_SUB(NOW(),INTERVAL 1 MINUTE) LIMIT 1",[target.quote_id||target.id||null,msg]);
    if(!dup.length) await pool.query("INSERT INTO admin_notifications(type,quote_id,message) VALUES('system',?,?)",[target.quote_id||target.id||null,msg]);
   }else if(rule.action_type==="email"){
    if(!app.locals.sendServiceEmail||!target.email){await log(rule,eventKey,target,"skipped","No hay servicio de email o destinatario.");return;}
    ...
    if(!app.locals.prepareWhatsappMessage||!target.phone){await log(rule,eventKey,target,"skipped","No hay servicio de WhatsApp o destinatario.");return;}
 ...
 async function runAutomation(eventKey,target={}){
  ...
  for(const rule of rules){
   ...
   if(delay>0){
    await log(rule,eventKey,target,"skipped","Automatización programada para ejecución diferida.");
    setTimeout(()=>executeRule(rule,eventKey,target).catch(console.error),delay*60000);
   }else await executeRule(rule,eventKey,target);
  }
 }
 ...
 app.get("/api/admin/automations/status",requireAdmin,async(req,res)=>{try{const [[r]]=await pool.query("SELECT COUNT(*) total,SUM(active=1) active FROM automation_rules");res.json({success:true,total:Number(r.total||0),active:Number(r.active||0)});}catch(e){res.status(500).json({error:"No se pudo obtener el estado de automatizaciones."});}});
}
